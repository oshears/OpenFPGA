...
from module_pkg.generate_partitions import *

# from module_pkg.windows_6x6 import *
...
def standard_flow(VERTICAL_CLB_COUNT):
    ...
    results_dir = f"openfpga_flow/tasks/basic_tests/0_debug_task/openfpga__arch_42x42__windows_1__partitions_400__tiered_luts__20240517/latest/k4_N4_tileable_40nm_new/bench0_fpga_design/MIN_ROUTE_CHAN_WIDTH"
    ...
    module_order = None
    with open(f'/home/oshears/Documents/openfpga/OpenFPGA/debug/architectures/arch_gen/results/openfpga__arch_42x42__windows_1__partitions_400__tiered_luts__20240517/_new_info/module_config_order.pkl',"rb") as file:
        module_order = pickle.load(file)
    
    print(module_order[0])
    print(module_order[-1])

    modules:dict[str, Module] = getModules(baseDir, bitstreamFile, module_order=module_order)
    # with open(f'/home/oshears/Documents/openfpga/OpenFPGA/debug/architectures/arch_gen/results/openfpga__arch_42x42__windows_1__partitions_400__tiered_luts__20240517/_new_info/module_info.pkl',"rb") as file:
    ...
    windows = generate_partitions(42)

    for window_idx in range(len(windows)):
        genNewBitstreams(modules, module_order, windows[window_idx], f"window{window_idx}_mappings.csv")
